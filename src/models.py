...
class DummyImageEncoder(nn.Module):
    def __init__(self, embed_size, backbone='resnet152'):
    ...
    def forward(self, image):
        ...
        linear = self.linear(self.bn(img_ft.reshape(img_ft.size(0), -1)))
#         out = linear
        out = self.linear_1(linear)
#         out = self.bn(linear2)
        return out
...
#         self.bn = nn.BatchNorm1d(vocab_embed_size, momentum=0.01)
    ...
    def forward(self, input, lengths):
        ...
        output = [padded[output_length[i]-1, i, :]
                  for i in range(len(output_length))]
        output = torch.stack([output[reverse_sort_ids[i]]
                              for i in range(len(output))], dim=0)
...
class MLP(nn.Module):
    # MLP for GAN embedding learning
    def __init__(self, hidden_layers, in_dim, out_dim, dropout_prob=0.5):
        super(MLP, self).__init__()
        if len(hidden_layers) != 2:
            raise ValueError('MLP can only have three hidden layers currently.')
        
        # Add concatenation layer 
        
        self.ln1 = nn.Linear(in_dim, hidden_layers[0])
        self.ln2 = nn.Linear(hidden_layers[0], hidden_layers[1])
        self.ln3 = nn.Linear(hidden_layers[1], 8192)
        self.ln4 = nn.Linear(8192, 8192)
        self.ln5 = nn.Linear(8192, out_dim)
        self.dropout1 = nn.Dropout(p=dropout_prob)
        self.dropout2 = nn.Dropout(p=dropout_prob)
        self.bn = nn.BatchNorm1d(hidden_layers[0], momentum=0.01)
        self.tanh = nn.Tanh()
        self.linear2 = nn.Linear(hidden_layers[0], out_dim)
