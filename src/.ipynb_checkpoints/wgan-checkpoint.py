...
def calc_gradient_penalty(netD, real_data, fake_data, batch_length, device, DIM=64, LAMBDA=10, caption_ft=None):
    ...
    alpha = alpha.expand(batch_length, int(real_data.nelement()/batch_length)).contiguous()
...
class Discriminator(nn.Module):
    def __init__(self, channels, dim):
        ...
        self.rb1 = ResidualBlock(self.dim, 2*self.dim, channels, resample = 'down', hw=self.dim)
        self.rb2 = ResidualBlock(2*self.dim, 4*self.dim, channels, resample = 'down', hw=int(self.dim/2))
        self.rb3 = ResidualBlock(4*self.dim, 8*self.dim, channels, resample = 'down', hw=int(self.dim/4))
        self.rb4 = ResidualBlock(8*self.dim, 8*self.dim, channels, resample = 'down', hw=int(self.dim/8))
