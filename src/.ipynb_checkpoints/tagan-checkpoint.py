...
class ReadFromVec(data.Dataset):
    def __init__(self, img_root, caption_root, classes_filename, img_transform=None):
        ...
        self.data = self._load_dataset(
            img_root, caption_root, classes_filename)
        ...
        with open(os.path.join(caption_root, classes_filename)) as f:
            ...
            for line in lines:
                ...
                filenames = os.listdir(
                    os.path.join(caption_root + '_vec', cls))
                for filename in filenames:
                    datum = torch.load(os.path.join(
                        caption_root + '_vec', cls, filename))
                    output.append({
                        'source_img': os.path.join(img_root, datum['source_img']),
                        'target_img': os.path.join(img_root, datum['target_img']),
    ...
    def __getitem__(self, index):
        ...
        target_img = img_load_and_transform(
            datum['target_img'], self.img_transform)
        source_img = img_load_and_transform(
            datum['source_img'], self.img_transform)
        word_vec = datum['word_vec']
        len_desc = datum['len_desc']
        ...
        return source_img, target_img, word_vec, len_desc
...
class Generator(nn.Module):
    ...
    def forward(self, img, txt):
        ...
        for i in range(txt_data.size(0)):
            ...
            hi_b = mask_i * \
                self.txt_encoder_b(
                    txt_data[-i - 1], hi_b) + (1 - mask_i) * hi_b
            h_b.append(hi_b)
        ...
        merge = self.residual_blocks(
            torch.cat((e, cond.repeat(1, 1, e.size(2), e.size(3))), 1))
...
class Discriminator(nn.Module):
    ...
    def forward(self, img, txt, len_txt, negative=False):
        ...
        img_feats = [self.GAP_1(img_feat_1), self.GAP_2(
            img_feat_2), self.GAP_3(img_feat_3)]
        ...
        idx_n = torch.tensor(
            np.roll(idx, 1), dtype=torch.long, device=txt.device)
        ...
        for i in range(3):
            ...
            if negative:
                ...
                sim_n += torch.sigmoid(torch.bmm(W_cond_n,
                                                 img_feat) + b_cond_n).squeeze(-1) * weight_n
            sim += torch.sigmoid(torch.bmm(W_cond, img_feat) +
                                 b_cond).squeeze(-1) * weight[i]

        if negative:
            att_txt_n = att_txt[:, idx_n]
            sim_n = torch.clamp(
                sim_n + self.eps, max=1).t().pow(att_txt_n).prod(0)
    ...
    def _encode_txt(self, txt, len_txt):
        ...
        for i in range(txt.size(0)):
            ...
            h_f.append(hi_f)
            hi_b = mask_i * \
                self.txt_encoder_b(txt[-i - 1], hi_b) + (1 - mask_i) * hi_b
            h_b.append(hi_b)
        mask = torch.stack(mask[::-1])
        h_f = torch.stack(h_f) * mask
        h_b = torch.stack(h_b[::-1])
