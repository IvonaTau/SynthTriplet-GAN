...
from utils import Ranker, ResnetRanker, img_load_and_transform
...
def compute_score(solution, prediction):
    ...
    for i in range(n):
        ...
        scores_r_10.append(
            recall(solution[i]["target"], prediction[i]["ranking"], 10))
        scores_r_50.append(
            recall(solution[i]["target"], prediction[i]["ranking"], 50))
...
def evaluate(args):
    ...
    image_encoder.load_state_dict(torch.load(
        image_model, map_location=device), strict=False)
    ...
    json.dump(output, open("../results/{}/{}.{}.pred.json".format(args.data_split,
                                                                  args.data_set, args.data_split), 'w'), indent=4)
    print('eval completed. Output file: {}'.format(
        "{}.{}.pred.json".format(args.data_set, args.data_split)))

    # Print out recall values
    try:
        print('Recall@10 , Recall@50 : ', compute_score(output, output))
    except:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_folder', type=str, default='../models/start-kit/dress-20190924-120426/',
